...
def leave_requests(request):
    current_emp = get_current_employee(request)
    is_manager  = request.user.is_staff or request.user.is_superuser or is_manager_or_above(current_emp)
...
def approve_leave(request, pk):
    current_emp = get_current_employee(request)
    if not (request.user.is_staff or request.user.is_superuser or is_manager_or_above(current_emp)):
